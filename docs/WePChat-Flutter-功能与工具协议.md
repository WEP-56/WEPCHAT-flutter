...
- 删除会话时同步删除其工作区目录及内容；该操作不可恢复。
- 所有内置文件、脚本和图片工具只能操作当前会话工作区。
- 内置工具不能通过参数访问工作区以外的文件路径。
- MCP 是用户明确启用的高级例外：外部工具不受工作区门禁限制，启用时展示说明，调用仍经过权限检查。Android 支持网络 MCP，Windows 额外支持 stdio，详见 [MCP 说明](MCP.md)。
...
- JS 只能访问当前会话工作区。
- JS 不接触真实设备路径。
- 开放标准 `fetch` 供网络连通性与 API 调试；不开放 DOM、Shell、进程、环境变量和系统 API。
- 脚本源码会进入工具调用记录，不应把长期凭据或私密 Key 直接写进脚本。
...
### 10.4 排队式引导（queued steering）

别名：排队式引导 / 任务中动态干预 / 运行中排队输入 / queued user input。业界没有统一叫法，pi 里叫 **steering messages**，本协议统一用「排队式引导」。

**要解决的问题**：一轮生成可能要跑十几个工具、几十秒。用户中途发现方向不对（"这个文件读错了"、"换成另一个目录"、"先别删"），当前只有两个选择——等它跑完再说，或者按停止从头来。前者浪费一整轮，后者丢掉已经产生的副作用。正确行为是**说一句、插进去、让它在下一个安全点读到**。

**一次插话的完整时序**：

```text
Agent 正在执行 tool A（可能还有 tool B、C 在同批）
  ↓
用户发送补充信息 / 调整要求
  ↓
① 消息立刻落库（entry，role = user）
② 进入 pending input queue，界面显示「排队中 · 这一步做完就发过去」
  ↓
tool A（以及同批其他工具）执行完毕，tool_result 全部进历史
  ↓
③ 排队消息出队，作为一条普通 user 消息并进历史，发 message_injected 事件
  ↓
④ Agent 带着「完整工具结果 + 一句新要求」发下一次请求：继续或重新规划
```

**两个注入点**（都只在"没有任何工具正在执行"时）：

1. **即将发请求之前**——上一批工具已经全部跑完、tool_result 已经进历史。这是主路径。
2. **模型本来要收场之前**——模型说了最后一句话、没有工具调用、即将结束这一轮。此时若队列非空，就**不收场**，把插话并进历史再发一次请求。

**硬规则**：

- **工具绝不半途打断**。插话不取消正在跑的工具，也不跳过同批剩下的工具。半途打断会让已经发生的副作用没有下文（文件删了、脚本跑了、MCP 请求发出去了），模型却看不到结果。
- **先落库，再入队**。用户按下发送，这条消息就不该有丢失路径。落库用的是和普通用户消息**完全同一套 payload**，所以即使这一轮没赶上注入点，下一轮读历史时它照样在上下文里。
- **取走即用**。loop 从队列取走一批就立刻并进历史并发注入事件，不攒着等下一轮开头。攒着的话，中途的取消 / 超时会让这一批"已经出队却没人用"——界面上的排队标记撤了，模型却没看见。
- **最后一轮不取**：取出也没有下一轮能注入，消息会从这一轮凭空消失。留在队列里交给会话层，会话层给一句「有 N 条消息没赶上这一轮，已保存，下次发消息会一起带上」。
- **用户按停止则不取**：用户自己的动作优先于任何收场判词，直接以中断收尾，队列留待会话层收尾。
- **队列归会话层所有，不归 loop**。谁在跑、气泡怎么显示、条目落到哪都是会话状态；loop 只持有「取队列」这一个回调（`AgentConfig.takePendingInputs`），为 null 就是不支持插话。
- **只对当前会话生效**。A 会话生成中，用户在 B 会话发消息照旧丢弃——插话只对"用户正看着的这一轮"有意义（全局单 run 是刻意简化）。
- **不产生新的 stopReason**。插话不是一种结束原因：插话后的新一轮走正常的 `stop` / `tool_use` 流程。

**界面约定**：

- 生成中输入框不禁用。发送按钮与停止按钮**并存**：停止是"别说了"，发送是"再补一句（等这一步做完）"。只剩停止的话，用户想插话就只能先停掉，代价是丢掉半轮工作。
- 排队气泡带「排队中」标记，且**不给操作栏**（复制 / 编辑 / 重发都要靠 `seq` 找落点，而它还没有）。
- 注入时这一轮的流式气泡**就地定格**，后续文字另起一条气泡，排在用户那句话**下面**。否则助手后面吐的字会跑到用户那句话上面去。
- 排队与注入都不触发 `_reload`——那会把正在生成的草稿冲掉。

**已知偏离与不做**：

- **不做队列模式 `all` / `oneAtATime`**（pi 有）。本项目固定为"取走当时队列里的全部"，同一检查点排了多条就按先后顺序一起注入。理由：多花一个开关，换不到用户能感知的差别。
- **不做 follow-up 队列**（pi 在外层循环里做）。pi 的 follow-up 是"agent 要停了才处理的下一条用户消息"，本项目里"agent 停了"就等于这一轮结束、回到界面等用户说话，不需要第二级队列。
- **不做排队消息撤回**。条目是 append-only 的，撤回要么改存储契约（加删条目），要么只删内存——后者会让这条消息在下一次读历史时"复活"，比不撤回更糟。用户要改主意可以直接再插一句。

**参照实现**（`example/pi`，TypeScript）：`packages/agent/src/agent-loop.ts` 的内层 `while (hasMoreToolCalls || pendingMessages.length > 0)` 就是注入点所在；`getSteeringMessages` 在每轮工具跑完后被调用，`getFollowUpMessages` 只在要收场时被调用。本项目只取其前半部分，循环结构简化为一层（见实施 TODO §5-1）。
...
- MCP 的资源浏览、prompts 与 OAuth 等扩展能力（tools 客户端已支持，见 MCP.md）
- 自定义工具包
- 更丰富的 JS 内置模块
- 工作区全文索引
- 可选的复杂项目执行后端
...
以下约束适用于默认能力；用户在高级功能中启用的外部 MCP server 由其自行信任和配置。WePChat 不承诺：
