...
**当前起点**：M2/M4 工具链已完成（工作区文件工具、搜索、图片、附件输入）。下一阶段转向工作区体验；Windows 不提供内置浏览器，HTML 文件交给系统默认浏览器。
...
**已完成**：写类工具（`write_file`、`edit_file`、`delete_file`）+ 纯聊天链路已换成 loop。
...
### 7.1 搜索工具（实施 TODO §7-15，M4，已完成）

**`web_search` / `web_fetch`** 已接通。后端路由集中在 `web_search`，搜索配置与聊天模型配置独立。
当前预设为 Tavily、Exa、Serper、SearXNG（自建），并支持自定义类型、Base URL 与 Key。
OpenAI/Anthropic 原生搜索不纳入当前方案。`web_fetch` 只 GET，不接受模型给的 header / cookie / Authorization。

搜索配置位于设置页“网页搜索”卡片。每个搜索服务可单独编辑、切换和删除（内置预设不可删除）。

### 7.2 图片工具（已完成）

`gen_image` 与 `edit_image` 已接 OpenAI Image API。前者从提示词从零生成，后者读取工作区图片后生成编辑结果，默认写入新文件且不覆盖原图。

### 7.3 附件输入（已完成）

聊天输入框支持 Windows/Android 文件选择、Windows 拖拽，以及 Windows/Android 标准 `Ctrl/⌘+V` 图片粘贴。附件会进入用户消息 payload，并按图片类型转换为 `ImagePart`。

### 7.4 长期记忆（实施 TODO §7-17，M6）
...
下一阶段聚焦工作区体验（Windows 不内置浏览器）：

- 预览能力与更多文件类型支持
- 附件上传与工作区导出
- `.html` / `.htm` 文件：Android 在应用内浏览器打开，Windows 交给系统默认浏览器；不做 Flutter 原生 HTML 模拟预览页
- 工作区文件增删查改能力完善
- Android 应用内浏览器单独实现：至少包含下载管理、浏览历史和历史清理；Windows 永远使用默认浏览器
- 工作区 UI 已支持创建、删除、上传、导出；文件查看器会优先读取真实工作区内容，HTML 在 Android 进入应用内浏览器

浏览器参考代码：`example/fluxdo` 仅作为 Android 应用内浏览器的后续参考；Windows 不引入 WebView2 或内置浏览器窗口。
...
## 11. 待用户拍的决策（实施 TODO §13）

已定的八条见实施 TODO §13，不重复。仍然悬着的：

- 搜索服务预设和自定义 Base URL/Key 已落地；后续如需更多后端再扩展统一路由。
- 删除会话要不要连带删工作区文件（见 §7 第 7 条）。
- 压缩自动触发（已定），但"上下文已压缩"的界面提示长什么样没设计。
- Android 应用内浏览器的设置入口与数据管理：下载列表、浏览历史、清理历史（待实现）。
