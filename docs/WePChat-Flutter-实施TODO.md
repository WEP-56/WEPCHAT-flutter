...
- [x] 5-1 一层循环就够（`AgentLoop.run`）：发请求 → 收流 → 有 tool_use 就执行、结果进历史、回到第一步。pi 的外层 follow-up 循环是为了它的 follow-up 队列——"agent 要停时才处理的下一条用户消息"。本项目"agent 停了"就是这一轮结束、回界面等用户说话，不需要第二级队列，所以插话检查点直接写在 for 里（协议 §10.4）。
- [x] 5-2 单次运行守卫：同一会话同时只允许一个 run。生成中输入框**不再禁用**——发送等于排队插话，停止按钮与发送按钮并存（协议 §10.4）。生成的是**别的**会话时，发送照旧丢弃，`SessionStore.sendMessage` 兜这一层。
- [ ] 5-3 ~~队列模式 `all` / `oneAtATime`~~ **不做**。固定为"取走当时队列里的全部"，同一检查点排了多条按先后顺序一起注入。多一个开关换不到用户能感知的差别。
- [x] 5-4 **排队式引导（插话）已实现**（协议 §10.4）。`AgentConfig.takePendingInputs` 回调 + `AgentInputInjected` 事件 + `_RunState.queue` + `ChatMessage.queued` 标记 + 「排队中」气泡；工具绝不半途打断，消息先落库再入队，取走即用，最后一轮与取消时不取。
...
- [x] 9-11 删除会话：软删 → 删 `blob_refs` → 清工作区目录（删除确认明确提示内容将一并删除）→ 硬删行。工作区清理失败时保留会话记录并返回可诊断错误，便于用户重试。
